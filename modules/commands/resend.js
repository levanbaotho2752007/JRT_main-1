  ...
  if (senderID == global.data.botID) return;
    ...
    if(event.type == "message_unsend") {
     ...
     let name = await Users.getNameUser(senderID);
      if(getMsg.attachment[0] == undefined) return api.sendMessage(`${name} đã che giấu 1 tin nhắn\nNội dung: ${getMsg.msgBody}`,threadID)
      else {
            let num = 0
            let msg = {
              body:`${name} vừa che giấu ${getMsg.attachment.length} tội lỗi:${(getMsg.msgBody != "") ? `\n\nNội dung: ${getMsg.msgBody}` : ""}`,
              attachment:[],
        ...
        var path = __dirname + `/resend/${random}.${ext}`;
