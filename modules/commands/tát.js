...
module.exports.onLoad = () => {
    ...
    if (!fs.existsSync(dirMaterial + "slap1.png")) request("https://i.imgur.com/PkyR7L2.png").pipe(fs.createWriteStream(dirMaterial + "slap1.png"));
}
...
async function makeImage({ one, two }) {    
    ...
    let slap_image = await jimp.read(__root + "/slap1.png");
...
module.exports.run = async function ({ event, api, args, client }) {
    ...
    if (!mention) return api.sendMessage("Vui lòng tag 1 người", threadID, messageID);
        ...
        return makeImage({ one, two }).then(path => api.sendMessage({ body: "Tát chết cmm nè", attachment: fs.createReadStream(path) }, threadID, () => fs.unlinkSync(path), messageID));
    }
}