...
module.exports.run = async ({ event, api, args, Currencies, Users }) => {
    ...
    let all = await Currencies.getAll(['userID', 'exp']);
				...
				for (var i = 0; i < 50; i++) {		       	//thay vào số line cần check		 
 ...
 if (args[0] == "money") { 
    let all = await Currencies.getAll(['userID', 'money']);
				...
				let num = 0;
	             let msg = {
					body: '⚡️Top 50 người dùng giàu nhất sever !',
	   ...
	   if (args[0] == "user") {
		...
		return api.sendMessage(`⚡️𝙏𝙤𝙥 ${option} 𝙣𝙜𝙪̛𝙤̛̀𝙞 𝙙𝙪̀𝙣𝙜 𝙡𝙖̆́𝙢 𝙢𝙤̂̀𝙢 𝙣𝙝𝙖̂́𝙩 𝙨𝙚𝙫𝙚𝙧 𝙘𝙪̉𝙖 𝘿𝙈𝙃 !\n\n${msg}`, threadID, messageID);
	}
