...
module.exports.run = async function ({ api, event, args, Users }) {
  ...
  let text = args.join(" ")
  if (!text) return api.sendMessage('‼️ Vui lòng nhập đúng định dạng text1 - text2 - text3 ❗', event.threadID, event.messageID);
  const length_0 = parseInt(text.length)
  const text1 = text.substr(0, text.indexOf(' - ')); 
  if (!text1) return api.sendMessage('‼️ Vui lòng nhập đúng định dạng text1 - text2 - text3 ❗', event.threadID, event.messageID);
  const length = parseInt(text1.length)
  const text3 = text.split(" - ").pop()
  if (!text3) return api.sendMessage('‼️ Vui lòng nhập đúng định dạng text1 - text2 - text3 ❗', event.threadID, event.messageID);
  const length_3 = parseInt(text3.length)
  const text2 = text.slice(length+2, length_0-length_3-2)
  if (!text2) return api.sendMessage('‼️ Vui lòng nhập đúng định dạng text1 - text2 - text3 ❗', event.threadID, event.messageID);
  let Avatar = (
    await axios.get(
