...
if(!isNaN(args[0])) {
        ...
        return api.sendMessage({ body: `Đã chuyển cho ${name} ${args[0]} đô`}, threadID, async () => {
...
if(!mention) return api.sendMessage('Vui lòng tag hoặc reply tin nhắn người muốn chuyển tiền cho!',threadID,messageID);
else {
	if(!isNaN(args[0+ name])) {
        ...
        return api.sendMessage({ body: 'Đã chuyển cho ' + event.mentions[mention].replace(/@/g, "") + ` ${args[0+ name]} đô`}, threadID, async () => {
            await Currencies.increaseMoney(mention, parseInt(coins));
                  Currencies.decreaseMoney(senderID, parseInt(coins));
            }, messageID);
		}
